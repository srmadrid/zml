...
/// types, where at least one operand must be of rational type, for equality.
/// The operation is performed by casting both operands to rational, then
/// comparing them.
///
/// ## Signature
/// ```zig
/// rational.eq(x: X, y: Y) Cmp
/// ```
///
/// ## Arguments
...
pub fn eq(x: anytype, y: anytype) bool {
    ...
    comptime if (!types.isNumeric(X) or !types.isNumeric(Y) or
        ...
        @compileError("zml.rational.eq: at least one of x or y must be a rational, the other must be a bool, an int, a float, a dyadic, an integer or a rational, got\n\tx: " ++
            @typeName(X) ++ "\n\ty: " ++ @typeName(Y) ++ "\n");

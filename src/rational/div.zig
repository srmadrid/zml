...
/// Performs division between two operands of rational, integer, cfloat, dyadic,
...
/// ## Signature
/// ```zig
/// rational.div(allocator: std.mem.Allocator, x: X, y: Y) !Rational
/// ```
///
/// ## Arguments
...
///
/// ## Returns
/// `Rational`: The result of the division.
///
/// ## Errors
/// * `std.mem.Allocator.Error.OutOfMemory`: If memory allocation fails.
/// * `rational.ZeroDivision`: If `y` is zero.
pub fn div(allocator: std.mem.Allocator, x: anytype, y: anytype) !Rational {
    ...
    comptime if (!types.isNumeric(X) or !types.isNumeric(Y) or
        ...
        @compileError("zml.rational.div: at least one of x or y must be a rational, the other must be a bool, an int, a float, a dyadic, a cfloat, an integer or a rational, got\n\tx: " ++
            @typeName(X) ++ "\n\ty: " ++ @typeName(Y) ++ "\n");

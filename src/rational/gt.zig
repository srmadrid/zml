...
pub fn gt(x: anytype, y: anytype) bool {
    ...
    comptime if (!(types.numericType(X) == .rational and types.numericType(Y) == .rational) and
        ...
        @compileError("rational.gt requires x or y to be a rational type, the other must be a rational, integer, float, int or bool type, got " ++ @typeName(X) ++ " and " ++ @typeName(Y));

...
pub fn neg_(o: anytype, x: anytype) void {
    ...
    if (comptime meta.isCustomNumeric(O)) {
        if (comptime meta.isCustomNumeric(X)) { // O and X both custom
            if (comptime meta.anyHasMethod(&.{ O, X }, "neg_", fn (*O, X) void, &.{ *O, X })) |Impl|
                return Impl.neg_(o, x);
        } else { // only O custom
            if (comptime meta.hasMethod(O, "neg_", fn (*O, X) void, &.{ *O, X }))
                return O.neg_(o, x);
        }
    } else if (comptime meta.isCustomNumeric(X)) { // only X custom
        if (comptime meta.hasMethod(X, "neg_", fn (*O, X) void, &.{ *O, X }))
            return X.neg_(o, x);
    }

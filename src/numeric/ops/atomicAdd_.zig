...
pub inline fn atomicAdd_(ptr: anytype, val: anytype) void {
    comptime var P = @TypeOf(ptr);
    const V = @TypeOf(val);

    comptime if (!meta.isPointer(P) or meta.isConstPointer(P) or !meta.isNumeric(meta.Child(P)) or
        !meta.isNumeric(V))
        @compileError("zsl.numeric.atomicAdd_: ptr must be a mutable one-item pointer to a numeric, and val must be a numeric, got \n\tptr: " ++ @typeName(P) ++ "\n\tval: " ++ @typeName(V) ++ "\n");

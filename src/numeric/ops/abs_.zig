...
pub fn abs_(o: anytype, x: anytype) void {
    ...
    if (comptime meta.isCustomNumeric(O)) {
        if (comptime meta.isCustomNumeric(X)) { // O and X both custom
            if (comptime meta.anyHasMethod(&.{ O, X }, "abs_", fn (*O, X) void, &.{ *O, X })) |Impl|
                return Impl.abs_(o, x);
        } else { // only O custom
            if (comptime meta.hasMethod(O, "abs_", fn (*O, X) void, &.{ *O, X }))
                return O.abs_(o, x);
        }
    } else if (comptime meta.isCustomNumeric(X)) { // only X custom
        if (comptime meta.hasMethod(X, "abs_", fn (*O, X) void, &.{ *O, X }))
            return X.abs_(o, x);
    }

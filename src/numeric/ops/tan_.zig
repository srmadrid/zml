...
pub fn tan_(o: anytype, x: anytype) void {
    ...
    if (comptime meta.isCustomNumeric(O)) {
        if (comptime meta.isCustomNumeric(X)) { // O and X both custom
            if (comptime meta.anyHasMethod(&.{ O, X }, "tan_", fn (*O, X) void, &.{ *O, X })) |Impl|
                return Impl.tan_(o, x);
        } else { // only O custom
            if (comptime meta.hasMethod(O, "tan_", fn (*O, X) void, &.{ *O, X }))
                return O.tan_(o, x);
        }
    } else if (comptime meta.isCustomNumeric(X)) { // only X custom
        if (comptime meta.hasMethod(X, "tan_", fn (*O, X) void, &.{ *O, X }))
            return X.tan_(o, x);
    }

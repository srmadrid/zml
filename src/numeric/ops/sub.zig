...
pub fn Sub(X: type, Y: type) type {
    ...
    switch (comptime types.numericType(X)) {
        .bool => switch (comptime types.numericType(Y)) {
            ...
            .complex => return complex.Sub(complex.Complex(rational.Rational), Y),
            .custom => unreachable,
        },
...
/// `X` must implement the required `Sub` and `sub` methods. The expected
/// signature and behavior of `Sub` and `sub` are as follows:
/// * `fn Sub(type) type`: Returns the return type of `sub` for the custom
///   numeric type.
/// * `X.Sub(X, Y)` is non-allocated: `fn sub(X, Y) X.Sub(X, Y)`: Returns the
///   subtraction of `x` and `y`.
/// * `X.Sub(X, Y)` is allocated: `fn sub(std.mem.Allocator, X, Y) !numeric.Sub(X, Y)`:
///   Returns the subtraction of `x` and `y` as a newly allocated value.
///
/// ### `Y` is a custom numeric type
/// `Y` must implement the required `Sub` and `sub` methods, with the same
/// specifications as above.
///
/// ### Both `X` and `Y` are custom numeric types
/// At least one of `X` and `Y` must implement the required `Sub` and `sub`
...
pub inline fn sub(x: anytype, y: anytype, ctx: anytype) !numeric.Sub(@TypeOf(x), @TypeOf(y)) {
    ...
    if (comptime types.isCustomType(X)) {
        if (comptime types.isCustomType(Y)) { // X and Y both custom
            if (comptime types.isAllocated(R)) {
                if (comptime !types.hasMethod(X, "sub", fn (std.mem.Allocator, X, Y) anyerror!R, &.{ std.mem.Allocator, X, Y })) {
                    comptime if (!types.hasMethod(Y, "sub", fn (std.mem.Allocator, X, Y) anyerror!R, &.{ std.mem.Allocator, X, Y }))
                        @compileError("zml.numeric.sub: " ++ @typeName(X) ++ " or " ++ @typeName(Y) ++ " must implement `fn sub(std.mem.Allocator, " ++ @typeName(X) ++ ", " ++ @typeName(Y) ++ ") !" ++ @typeName(R) ++ "`");
                    ...
                    return Y.sub(ctx.allocator, x, y);
                } else {
                    comptime types.validateContext(
                        @TypeOf(ctx),
                        .{
                    ...
                    return X.sub(ctx.allocator, x, y);
                }
            } else {
                if (comptime !types.hasMethod(X, "sub", fn (X, Y) R, &.{ X, Y })) {
                    comptime if (!types.hasMethod(Y, "sub", fn (X, Y) R, &.{ X, Y }))
                        @compileError("zml.numeric.sub: " ++ @typeName(X) ++ " or " ++ @typeName(Y) ++ " must implement `fn sub(" ++ @typeName(X) ++ ", " ++ @typeName(Y) ++ ") " ++ @typeName(R) ++ "`");
            ...
            if (comptime types.isAllocated(R)) {
                comptime if (!types.hasMethod(X, "sub", fn (std.mem.Allocator, X, Y) anyerror!R, &.{ std.mem.Allocator, X, Y }))
                    @compileError("zml.numeric.sub: " ++ @typeName(X) ++ " must implement `fn sub(std.mem.Allocator, " ++ @typeName(X) ++ ", " ++ @typeName(Y) ++ ") !" ++ @typeName(R) ++ "`");
                ...
                comptime if (!types.hasMethod(X, "sub", fn (X, Y) R, &.{ X, Y }))
                    @compileError("zml.numeric.sub: " ++ @typeName(Y) ++ " must implement `fn sub(" ++ @typeName(X) ++ ", " ++ @typeName(Y) ++ ") " ++ @typeName(R) ++ "`");
        ...
        if (comptime types.isAllocated(R)) {
            comptime if (!types.hasMethod(Y, "sub", fn (std.mem.Allocator, X, Y) anyerror!R, &.{ std.mem.Allocator, X, Y }))
                @compileError("zml.numeric.sub: " ++ @typeName(Y) ++ " must implement `fn sub(std.mem.Allocator, " ++ @typeName(X) ++ ", " ++ @typeName(Y) ++ ") !" ++ @typeName(R) ++ "`");

const meta = @import("../../meta.zig");

const int = @import("../../int.zig");
const float = @import("../../float.zig");
const dyadic = @import("../../dyadic.zig");
const complex = @import("../../complex.zig");

const numeric = @import("../../numeric.zig");

pub fn Sub(X: type, Y: type) type {
    comptime if (!meta.isNumeric(X) or !meta.isNumeric(Y))
        @compileError("zsl.numeric.Sub: X and Y must be numeric types, got \n\tX = " ++ @typeName(X) ++ "\n\tY = " ++ @typeName(Y) ++ "\n");

    if (comptime meta.isCustomNumeric(X)) {
        if (comptime meta.isCustomNumeric(Y)) { // X and Y both custom
            const Impl: type = comptime meta.anyHasMethod(
                &.{ X, Y },
                "Sub",
                fn (type, type) type,
                &.{ X, Y },
            ) orelse
                @compileError("zsl.numeric.Sub: " ++ @typeName(X) ++ " or " ++ @typeName(Y) ++ " must implement `fn Sub(type, type) type`");

            return Impl.Sub(X, Y);
        } else { // only X custom
            comptime if (!meta.hasMethod(X, "Sub", fn (type, type) type, &.{ X, Y }))
                @compileError("zsl.numeric.Sub: " ++ @typeName(X) ++ " must implement `fn Sub(type, type) type`");

            return X.Sub(X, Y);
        }
    } else if (comptime meta.isCustomNumeric(Y)) { // only Y custom
        comptime if (!meta.hasMethod(Y, "Sub", fn (type, type) type, &.{ X, Y }))
            @compileError("zsl.numeric.Sub: " ++ @typeName(Y) ++ " must implement `fn Sub(type, type) type`");

        return Y.Sub(X, Y);
    }

    switch (comptime meta.numericType(X)) {
        .bool => switch (comptime meta.numericType(Y)) {
            .bool => @compileError("zsl.numeric.Sub: not defined for " ++ @typeName(X) ++ " and " ++ @typeName(Y) ++ "."),
            .int => return int.Sub(X, Y),
            .float => return float.Sub(X, Y),
            .dyadic => return dyadic.Sub(X, Y),
            .complex => return complex.Sub(X, Y),
            .custom => unreachable,
        },
        .int => switch (comptime meta.numericType(Y)) {
            .bool, .int => return int.Sub(X, Y),
            .float => return float.Sub(X, Y),
            .dyadic => return dyadic.Sub(X, Y),
            .complex => return complex.Sub(X, Y),
            .custom => unreachable,
        },
        .float => switch (comptime meta.numericType(Y)) {
            .bool, .int, .float => return float.Sub(X, Y),
            .dyadic => return dyadic.Sub(X, Y),
            .complex => return complex.Sub(X, Y),
            .custom => unreachable,
        },
        .dyadic => switch (comptime meta.numericType(Y)) {
            .bool, .int, .float, .dyadic => return dyadic.Sub(X, Y),
            .complex => return complex.Sub(X, Y),
            .custom => unreachable,
        },
        .complex => switch (comptime meta.numericType(Y)) {
            .bool, .int, .float, .dyadic, .complex => return complex.Sub(X, Y),
            .custom => unreachable,
        },
        .custom => unreachable,
    }
}

/// Performs subtraction between any two numeric operands.
///
/// ## Signature
/// ```zig
/// numeric.sub(x: X, y: Y) numeric.Sub(X, Y)
/// ```
///
/// ## Arguments
/// * `x` (`anytype`): The left operand.
/// * `y` (`anytype`): The right operand.
///
/// ## Returns
/// `numeric.Sub(@TypeOf(x), @TypeOf(y))`: The result of the subtraction.
///
/// ## Custom type support
/// This function supports custom numeric types via specific method
/// implementations.
///
/// `X` or `Y` must implement the required `Sub` method. The expected
/// signature and behavior of `Sub` are as follows:
/// * `fn Sub(type, type) type`: Returns the type of `x - y`.
///
/// `numeric.Sub(X, Y)`, `X` or `Y` must implement the required `sub` method.
/// The expected signatures and behavior of `sub` are as follows:
/// * `fn sub(X, Y) numeric.Sub(X, Y)`: Returns the subtraction of `x` and
///   `y`.
pub fn sub(x: anytype, y: anytype) numeric.Sub(@TypeOf(x), @TypeOf(y)) {
    const X: type = @TypeOf(x);
    const Y: type = @TypeOf(y);
    const R: type = numeric.Sub(X, Y);

    if (comptime meta.isCustomNumeric(X)) {
        if (comptime meta.isCustomNumeric(Y)) { // X and Y both custom
            const Impl: type = comptime meta.anyHasMethod(
                &.{ R, X, Y },
                "sub",
                fn (X, Y) R,
                &.{ X, Y },
            ) orelse
                @compileError("zsl.numeric.sub: " ++ @typeName(R) ++ ", " ++ @typeName(X) ++ " or " ++ @typeName(Y) ++ " must implement `fn sub(" ++ @typeName(X) ++ ", " ++ @typeName(Y) ++ ") " ++ @typeName(R) ++ "`");

            return Impl.sub(x, y);
        } else { // only X custom
            const Impl: type = comptime meta.anyHasMethod(
                &.{ R, X },
                "sub",
                fn (X, Y) R,
                &.{ X, Y },
            ) orelse
                @compileError("zsl.numeric.sub: " ++ @typeName(R) ++ " or " ++ @typeName(X) ++ " must implement `fn sub(" ++ @typeName(X) ++ ", " ++ @typeName(Y) ++ ") " ++ @typeName(R) ++ "`");

            return Impl.sub(x, y);
        }
    } else if (comptime meta.isCustomNumeric(Y)) { // only Y custom
        const Impl: type = comptime meta.anyHasMethod(
            &.{ R, Y },
            "sub",
            fn (X, Y) R,
            &.{ X, Y },
        ) orelse
            @compileError("zsl.numeric.sub: " ++ @typeName(R) ++ " or " ++ @typeName(Y) ++ " must implement `fn sub(" ++ @typeName(X) ++ ", " ++ @typeName(Y) ++ ") " ++ @typeName(R) ++ "`");

        return Impl.sub(x, y);
    }

    switch (comptime meta.numericType(X)) {
        .bool => switch (comptime meta.numericType(Y)) {
            .bool => unreachable,
            .int => return int.sub(x, y),
            .float => return float.sub(x, y),
            .dyadic => return dyadic.sub(x, y),
            .complex => return complex.sub(x, y),
            .custom => unreachable,
        },
        .int => switch (comptime meta.numericType(Y)) {
            .bool, .int => return int.sub(x, y),
            .float => return float.sub(x, y),
            .dyadic => return dyadic.sub(x, y),
            .complex => return complex.sub(x, y),
            .custom => unreachable,
        },
        .float => switch (comptime meta.numericType(Y)) {
            .bool, .int, .float => return float.sub(x, y),
            .dyadic => return dyadic.sub(x, y),
            .complex => return complex.sub(x, y),
            .custom => unreachable,
        },
        .dyadic => switch (comptime meta.numericType(Y)) {
            .bool, .int, .float, .dyadic => return dyadic.sub(x, y),
            .complex => return complex.sub(x, y),
            .custom => unreachable,
        },
        .complex => switch (comptime meta.numericType(Y)) {
            .bool, .int, .float, .dyadic, .complex => return complex.sub(x, y),
            .custom => unreachable,
        },
        .custom => unreachable,
    }
}

/// Performs computation of the subtraction of two numerics `x` and `y` into a
/// numeric `o`.
///
/// ## Signature
/// ```zig
/// numeric.subInto(o: *O, x: X, y: Y) void
/// ```
///
/// ## Arguments
/// * `o` (`anytype`): The output operand.
/// * `x` (`anytype`): The left operand.
/// * `y` (`anytype`): The right operand.
///
/// ## Returns
/// `void`
///
/// ## Custom type support
/// This function supports custom numeric types via specific method
/// implementations.
///
/// `O`, `X` or `Y` should implement the required `subInto` method. The expected
/// signature and behavior of `subInto` are as follows:
/// * `fn subInto(*O, X, Y) void`: Computes the subtraction of `x` and `y` and
///   stores it in `o`.
///
/// If none of `O`, `X` and `Y` implement the required `subInto` method, the
/// function will fall back to using `numeric.set` with the result of
/// `numeric.sub`, potentially resulting in a less efficient implementation. In
/// this case, `O`, `X` and `Y` must adhere to the requirements of these functions.
pub fn subInto(o: anytype, x: anytype, y: anytype) void {
    comptime var O: type = @TypeOf(o);
    const X: type = @TypeOf(x);
    const Y: type = @TypeOf(y);

    comptime if (!meta.isPointer(O) or meta.isConstPointer(O) or
        !meta.isNumeric(meta.Child(O)) or
        !meta.isNumeric(X) or
        !meta.isNumeric(Y))
        @compileError("zsl.numeric.subInto: o must be a mutable one-item pointer to a numeric, and x and y must be numerics, got \n\to: " ++ @typeName(O) ++ "\n\tx: " ++ @typeName(X) ++ "\n\ty: " ++ @typeName(Y) ++ "\n");

    O = meta.Child(O);

    if (comptime meta.isCustomNumeric(O)) {
        if (comptime meta.isCustomNumeric(X)) {
            if (comptime meta.isCustomNumeric(Y)) { // O, X and Y all custom
                if (comptime meta.anyHasMethod(&.{ O, X, Y }, "subInto", fn (*O, X, Y) void, &.{ *O, X, Y })) |Impl|
                    return Impl.subInto(o, x, y);
            } else { // only O and X custom
                if (comptime meta.anyHasMethod(&.{ O, X }, "subInto", fn (*O, X, Y) void, &.{ *O, X, Y })) |Impl|
                    return Impl.subInto(o, x, y);
            }
        } else {
            if (comptime meta.isCustomNumeric(Y)) { // only O and Y custom
                if (comptime meta.anyHasMethod(&.{ O, Y }, "subInto", fn (*O, X, Y) void, &.{ *O, X, Y })) |Impl|
                    return Impl.subInto(o, x, y);
            } else { // only O custom
                if (comptime meta.hasMethod(O, "subInto", fn (*O, X, Y) void, &.{ *O, X, Y }))
                    return O.subInto(o, x, y);
            }
        }
    } else {
        if (comptime meta.isCustomNumeric(X)) {
            if (comptime meta.isCustomNumeric(Y)) { // only X and Y custom
                if (comptime meta.anyHasMethod(&.{ X, Y }, "subInto", fn (*O, X, Y) void, &.{ *O, X, Y })) |Impl|
                    return Impl.subInto(o, x, y);
            } else { // only X custom
                if (comptime meta.hasMethod(X, "subInto", fn (*O, X, Y) void, &.{ *O, X, Y }))
                    return X.subInto(o, x, y);
            }
        } else if (comptime meta.isCustomNumeric(Y)) { // only Y custom
            if (comptime meta.hasMethod(Y, "subInto", fn (*O, X, Y) void, &.{ *O, X, Y }))
                return Y.subInto(o, x, y);
        }
    }

    return numeric.set(o, numeric.sub(x, y));
}

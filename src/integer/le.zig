...
/// least one operand must be of integer type, for less-than or equal ordering.
/// The operation is performed by casting both operands to integer, then
/// comparing them.
///
/// ## Signature
/// ```zig
/// integer.le(x: X, y: Y) bool
/// ```
///
/// ## Arguments
...
pub fn le(x: anytype, y: anytype) bool {
    ...
    comptime if (!types.isNumeric(X) or !types.isNumeric(Y) or
        ...
        @compileError("zml.integer.le: at least one of x or y must be an integer, the other must be a bool, an int, a float, a dyadic or an integer, got\n\tx: " ++
            @typeName(X) ++ "\n\ty: " ++ @typeName(Y) ++ "\n");

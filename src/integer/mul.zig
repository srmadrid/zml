...
/// Performs multiplication between two operands of integer, cfloat, dyadic,
...
/// ## Signature
/// ```zig
/// integer.mul(allocator: std.mem.Allocator, x: X, y: Y) !Integer
/// ```
///
/// ## Arguments
...
pub fn mul(allocator: std.mem.Allocator, x: anytype, y: anytype) !Integer {
    ...
    comptime if (!types.isNumeric(X) or !types.isNumeric(Y) or
        ...
        @compileError("zml.integer.mul: at least one of x or y must be an integer, the other must be a bool, an int, a float, a dyadic, a cfloat or an integer, got\n\tx: " ++
            @typeName(X) ++ "\n\ty: " ++ @typeName(Y) ++ "\n");

...
/// Performs in-place division between two operands of integer, cfloat, dyadic,
...
/// ## Signature
/// ```zig
/// integer.div_(allocator: std.mem.Allocator, o: *Integer, x: X, y: Y) !void
/// ```
///
/// ## Arguments
...
/// * `integer.Error.ZeroDivision`: If `y` is zero.
...
pub fn div_(allocator: std.mem.Allocator, o: *Integer, x: anytype, y: anytype) !void {
    ...
    comptime if (!types.isNumeric(X) or !types.isNumeric(Y) or
        ...
        @compileError("zml.integer.div_: at least one of x or y must be an integer, the other must be a bool, an int, a float, a dyadic, a cfloat or an integer, got\n\tx: " ++
            @typeName(X) ++ "\n\ty: " ++ @typeName(Y) ++ "\n");
        ...
        .integer => switch (comptime types.numericType(Y)) {
            .integer => {
                ...
                if (integer.lt(
                    ...
                    integer.abs(null, y) catch unreachable,
                ))

...
pub fn apply2(
    ...
    var result: matrix.triangular.Dense(R, types.uploOf(Y), .non_unit, types.orderOf(Y)) = try .init(allocator, x.rows, x.cols);
            ...
            while (j < x.cols) : (j += 1) {
                ...
                if (j < int.min(x.rows, x.cols)) {
                    if (comptime types.diagOf(@TypeOf(y)) == .unit) {
                        if (comptime opinfo.@"fn".params.len == 2) {
                            result.data[j + j * result.ld] = op(x.data[j], constants.one(Y, ctx) catch unreachable);
                        } else if (comptime opinfo.@"fn".params.len == 3) {
                            result.data[j + j * result.ld] = try op(x.data[j], constants.one(Y, ctx) catch unreachable, ctx);
                        }
                    } else {
                        if (comptime opinfo.@"fn".params.len == 2) {
        ...
        if (comptime types.uploOf(@TypeOf(y)) == .upper) { // ru d ru
            ...
            while (i < x.rows) : (i += 1) {
                if (i < int.min(x.rows, x.cols)) {
                    if (comptime types.diagOf(@TypeOf(y)) == .unit) {
                        if (comptime opinfo.@"fn".params.len == 2) {
                            result.data[i * result.ld + i] = op(x.data[i], constants.one(Y, ctx) catch unreachable);
                        } else if (comptime opinfo.@"fn".params.len == 3) {
                            result.data[i * result.ld + i] = try op(x.data[i], constants.one(Y, ctx) catch unreachable, ctx);
                        }
                    } else {
                        if (comptime opinfo.@"fn".params.len == 2) {

                ...
                while (j < x.cols) : (j += 1) {
                    ...
                    while (i < j) : (i += 1) {
                        if (comptime opinfo.@"fn".params.len == 2) {
                            ...
                            result.data[j + i * result.ld] = op(x.data[j + i * x.ld], y.data[i + j * y.ld]);
                        } else if (comptime opinfo.@"fn".params.len == 3) {
                            result.data[i + j * result.ld] = try op(x.data[i + j * x.ld], y.data[i + j * y.ld], ctx);
                            result.data[j + i * result.ld] = try op(x.data[j + i * x.ld], y.data[i + j * y.ld], ctx);
                        }
                    }
                ...
                while (j < x.cols) : (j += 1) {
                    ...
                    while (i < x.rows) : (i += 1) {
                        if (comptime opinfo.@"fn".params.len == 2) {
                            ...
                            result.data[j + i * result.ld] = op(x.data[j + i * x.ld], y.data[j * y.ld + i]);
                        } else if (comptime opinfo.@"fn".params.len == 3) {
                            result.data[i + j * result.ld] = try op(x.data[i + j * x.ld], y.data[j * y.ld + i], ctx);
                            result.data[j + i * result.ld] = try op(x.data[j + i * x.ld], y.data[j * y.ld + i], ctx);
                        }
                    }
                }
            } else { // c c rl
                ...
                while (i < x.rows) : (i += 1) {
                    ...
                    while (j < i) : (j += 1) {
                        if (comptime opinfo.@"fn".params.len == 2) {
                            ...
                            result.data[j * result.ld + i] = op(x.data[j * x.ld + i], y.data[j + i * y.ld]);
                        } else if (comptime opinfo.@"fn".params.len == 3) {
                            result.data[i * result.ld + j] = try op(x.data[i * x.ld + j], y.data[j + i * y.ld], ctx);
                            result.data[j * result.ld + i] = try op(x.data[j * x.ld + i], y.data[j + i * y.ld], ctx);
                        }
                    }
                ...
                while (i < x.rows) : (i += 1) {
                    ...
                    while (j < x.cols) : (j += 1) {
                        if (comptime opinfo.@"fn".params.len == 2) {
                            ...
                            result.data[j * result.ld + i] = op(x.data[j * x.ld + i], y.data[i * y.ld + j]);
                        } else if (comptime opinfo.@"fn".params.len == 3) {
                            result.data[i * result.ld + j] = try op(x.data[i * x.ld + j], y.data[i * y.ld + j], ctx);
                            result.data[j * result.ld + i] = try op(x.data[j * x.ld + i], y.data[i * y.ld + j], ctx);
                        }
                    }
                }
            } else { // r r rl

        ...
        while (j < y.cols) : (j += 1) {
            ...
            while (p < y.ptr[j + 1]) : (p += 1) {
                if (comptime meta.uploOf(O) == meta.uploOf(Y)) {
                    ...
                    if (y.idx[p] == j) {
                        op_(&o.data[o._index(j, y.idx[p])], x.data[j], numeric.conj(y.data[p]));
                    } else {
                        if (comptime op_ == numeric.addInto)
                            numeric.set(&o.data[o._index(j, y.idx[p])], numeric.conj(y.data[p]))
                        else
                            numeric.set(&o.data[o._index(j, y.idx[p])], numeric.neg(numeric.conj(y.data[p])));
                    }
                }
            }
        }
        ...
        while (i < y.rows) : (i += 1) {
            ...
            while (p < y.ptr[i + 1]) : (p += 1) {
                if (comptime meta.uploOf(O) == meta.uploOf(Y)) {
                    ...
                    if (i == y.idx[p]) {
                        op_(&o.data[o._index(y.idx[p], i)], x.data[i], numeric.conj(y.data[p]));
                    } else {
                        if (comptime op_ == numeric.addInto)
                            numeric.set(&o.data[o._index(y.idx[p], i)], numeric.conj(y.data[p]))
                        else
                            numeric.set(&o.data[o._index(y.idx[p], i)], numeric.neg(numeric.conj(y.data[p])));
                    }
                }
            }
        }

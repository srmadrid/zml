        ...
        while (j < x.cols) : (j += 1) {
            ...
            while (p < x.ptr[j + 1]) : (p += 1) {
                if ((comptime op_ == numeric.addInto) or !aliased) {
                    ...
                    if (x.idx[p] != j) {
                        op_(&o.data[o._index(j, x.idx[p])], x.data[p], y.data[y._index(j, x.idx[p])]);
                    }
                } else {
                    op_(&o.data[o._index(x.idx[p], j)], x.data[p], numeric.neg(y.data[y._index(x.idx[p], j)]));

                    if (x.idx[p] != j) {
                        op_(&o.data[o._index(j, x.idx[p])], x.data[p], numeric.neg(y.data[y._index(j, x.idx[p])]));
                    }
                }
            }
        }
        ...
        while (i < x.rows) : (i += 1) {
            ...
            while (p < x.ptr[i + 1]) : (p += 1) {
                if ((comptime op_ == numeric.addInto) or !aliased) {
                    ...
                    if (i != x.idx[p]) {
                        op_(&o.data[o._index(x.idx[p], i)], x.data[p], y.data[y._index(x.idx[p], i)]);
                    }
                } else {
                    op_(&o.data[o._index(i, x.idx[p])], x.data[p], numeric.neg(y.data[y._index(i, x.idx[p])]));

                    if (i != x.idx[p]) {
                        op_(&o.data[o._index(x.idx[p], i)], x.data[p], numeric.neg(y.data[y._index(x.idx[p], i)]));
                    }
                }
            }
        }

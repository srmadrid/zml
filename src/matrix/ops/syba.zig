...
pub fn apply2(
    ...
    if (comptime types.orderOf(@TypeOf(x)) == .col_major) { // orderOf(result) == orderOf(x)
        if (comptime types.uploOf(@TypeOf(x)) == .upper) { // c cu c/r
            var j: u32 = 0;
            while (j < x.size) : (j += 1) {
                var i: u32 = 0;
                while (i < j) : (i += 1) {
                    if (comptime opinfo.@"fn".params.len == 2) {
                        ...
                        result.data[j + i * result.ld] = op(x.data[i + j * x.ld], y.get(j, i) catch unreachable);
                    } else if (comptime opinfo.@"fn".params.len == 3) {
                        result.data[i + j * result.ld] = try op(x.data[i + j * x.ld], y.get(i, j) catch unreachable, ctx);
                        result.data[j + i * result.ld] = try op(x.data[i + j * x.ld], y.get(j, i) catch unreachable, ctx);
                    }
                }
        ...
        if (comptime types.uploOf(@TypeOf(x)) == .upper) { // r ru c/r
            var i: u32 = 0;
            while (i < x.size) : (i += 1) {
                if (comptime opinfo.@"fn".params.len == 2) {
                ...
                while (j < x.size) : (j += 1) {
                    if (comptime opinfo.@"fn".params.len == 2) {
                        ...
                        result.data[j * result.ld + i] = op(x.data[i * x.ld + j], y.get(j, i) catch unreachable);
                    } else if (comptime opinfo.@"fn".params.len == 3) {
                        result.data[i * result.ld + j] = try op(x.data[i * x.ld + j], y.get(i, j) catch unreachable, ctx);
                        result.data[j * result.ld + i] = try op(x.data[i * x.ld + j], y.get(j, i) catch unreachable, ctx);
                    }
                }
            }
        } else { // r rl c/r

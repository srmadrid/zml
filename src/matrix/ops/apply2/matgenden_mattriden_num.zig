...
pub fn apply2IntoUnchecked(o: anytype, x: anytype, y: anytype, comptime opInto: anytype) void {
    ...
    if (comptime meta.layoutOf(O) == .col_major) {
        ...
        while (j < o.cols) : (j += 1) {
            ...
            if (comptime meta.uploOf(X) == .upper) {
                while (i < int.min(j, o.rows)) : (i += 1) {
                    opInto(&o.data[o._index(i, j)], x.data[x._index(i, j)], y);
                }
            } else {
                while (i < int.min(j, o.rows)) : (i += 1) {
            ...
            if (j < o.rows) {
                if (comptime meta.diagOf(X) == .unit)
                    opInto(&o.data[o._index(j, j)], numeric.one(meta.Numeric(X)), y)
                else
        ...
        while (i < o.rows) : (i += 1) {
            ...
            if (i < o.cols) {
                if (comptime meta.diagOf(X) == .unit)
                    opInto(&o.data[o._index(i, i)], numeric.one(meta.Numeric(X)), y)
                else

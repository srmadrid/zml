...
pub fn apply2(
    ...
    var result: matrix.Banded(R, types.orderOf(Y)) = try .init(allocator, x.size, x.size, int.max(y.lower, 1), int.max(y.upper, 1)); // at least one sub/super diagonal
    ...
    if (comptime types.orderOf(@TypeOf(y)) == .col_major) { // orderOf(result) == orderOf(y)
        ...
        while (j < x.size) : (j += 1) {
            ...
            while (i <= int.min(x.size - 1, j + result.lower)) : (i += 1) {
        ...
        while (i < x.size) : (i += 1) {
            ...
            while (j <= int.min(x.size - 1, i + result.upper)) : (j += 1) {
                if (comptime opinfo.@"fn".params.len == 2) {

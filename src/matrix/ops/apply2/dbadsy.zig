...
pub fn apply2(
    ...
    var result: matrix.general.Dense(R, types.orderOf(Y)) = try .init(allocator, x.rows, x.cols);
                ...
                while (j < x.cols) : (j += 1) {
                    ...
                    while (i < j) : (i += 1) {
                        if (comptime opinfo.@"fn".params.len == 2) {
                            ...
                            result.data[j + i * result.ld] = op(x.get(j, i) catch unreachable, y.data[i * y.ld + j]);
                        } else if (comptime opinfo.@"fn".params.len == 3) {
                            result.data[i + j * result.ld] = try op(x.get(i, j) catch unreachable, y.data[i * y.ld + j], ctx);
                            result.data[j + i * result.ld] = try op(x.get(j, i) catch unreachable, y.data[i * y.ld + j], ctx);
                        }
                    }
                ...
                while (i < x.rows) : (i += 1) {
                    ...
                    while (j < x.cols) : (j += 1) {
                        if (comptime opinfo.@"fn".params.len == 2) {
                            ...
                            result.data[j * result.ld + i] = op(x.get(j, i) catch unreachable, y.data[i + j * y.ld]);
                        } else if (comptime opinfo.@"fn".params.len == 3) {
                            result.data[i * result.ld + j] = try op(x.get(i, j) catch unreachable, y.data[i + j * y.ld], ctx);
                            result.data[j * result.ld + i] = try op(x.get(j, i) catch unreachable, y.data[i + j * y.ld], ctx);
                        }
                    }
                }
            } else { // r r cl

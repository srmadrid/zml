...
pub fn Dense(N: type, uplo: matrix.Uplo, layout: matrix.Layout) type {
    ...
    return struct {
        ...
        pub fn format(self: matrix.hermitian.Dense(N, uplo, layout), writer: *std.Io.Writer) !void {
            try self.formatter("{e}").format(writer);
        }

        pub fn formatter(self: *const matrix.hermitian.Dense(N, uplo, layout), comptime num_fmt: []const u8) Formatter(num_fmt) {
            return .{ .matrix = self };
        }

        pub fn Formatter(comptime num_fmt: []const u8) type {
            return struct {
                matrix: *const matrix.hermitian.Dense(N, uplo, layout),

                pub fn format(self: matrix.hermitian.Dense(N, uplo, layout).Formatter(num_fmt), writer: *std.Io.Writer) !void {
                    const rows = self.matrix.rows;
                    const cols = self.matrix.cols;

                    try writer.print("zsl.matrix.hermitian.Dense({s}, .{s}, .{s}) ({d} × {d}):\n\n", .{ @typeName(N), @tagName(uplo), @tagName(layout), rows, cols });

...
pub fn Static(size_: comptime_int, N: type, uplo: matrix.Uplo, layout: matrix.Layout) type {
    ...
    return struct {
        ...
        pub fn format(self: matrix.hermitian.Static(size, N, uplo, layout), writer: *std.Io.Writer) !void {
            try self.formatter("{e}").format(writer);
        }

        pub fn formatter(self: *const matrix.hermitian.Static(size, N, uplo, layout), comptime num_fmt: []const u8) Formatter(num_fmt) {
            return .{ .matrix = self };
        }

        pub fn Formatter(comptime num_fmt: []const u8) type {
            return struct {
                matrix: *const matrix.hermitian.Static(size, N, uplo, layout),

                pub fn format(self: matrix.hermitian.Static(size, N, uplo, layout).Formatter(num_fmt), writer: *std.Io.Writer) !void {
                    try writer.print("zsl.matrix.hermitian.Static({d}, {s}, .{s}, .{s}) ({d} × {d}):\n\n", .{ size, @typeName(N), @tagName(uplo), @tagName(layout), rows, cols });

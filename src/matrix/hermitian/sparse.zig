    ...
    return struct {
        ...
        pub fn set(self: *matrix.hermitian.Sparse(N, uplo, layout), r: usize, c: usize, value: N) !void {
            ...
            while (left < right) {
                ...
                if (mid_idx == minor) {
                    self.data[mid] = if (noconj == self.flags.noconj)
                        value
                    else
                        numeric.conj(value);
                    return;
        ...
        pub fn format(self: matrix.hermitian.Sparse(N, uplo, layout), writer: *std.Io.Writer) !void {
            try self.formatter("{e}").format(writer);
        }

        pub fn formatter(self: *const matrix.hermitian.Sparse(N, uplo, layout), comptime num_fmt: []const u8) Formatter(num_fmt) {
            return .{ .matrix = self };
        }

        pub fn Formatter(comptime num_fmt: []const u8) type {
            return struct {
                matrix: *const matrix.hermitian.Sparse(N, uplo, layout),

                pub fn format(self: matrix.hermitian.Sparse(N, uplo, layout).Formatter(num_fmt), writer: *std.Io.Writer) !void {
                    const rows = self.matrix.rows;
                    const cols = self.matrix.cols;

                    try writer.print("zsl.matrix.hermitian.Sparse({s}, .{s}, .{s}) ({d} × {d}):\n\n", .{ @typeName(N), @tagName(uplo), @tagName(layout), rows, cols });

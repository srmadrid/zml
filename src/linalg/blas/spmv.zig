...
/// Computes a matrix-vector product with a symmetric packed matrix defined as:
///
/// ```zig
/// y = alpha * A * x + beta * y
/// ```
...
/// ## Signature
/// ```zig
/// linalg.blas.spmv(layout: Layout, uplo: Uplo, n: usize, alpha: Al, ap: [*]const Ap, x: [*]const X, incx: isize, beta: Be, y: [*]Y, incy: isize) !void
/// ```
///
/// ## Arguments
...
pub fn spmv(layout: Layout, uplo: Uplo, n: usize, alpha: anytype, ap: anytype, x: anytype, incx: isize, beta: anytype, y: anytype, incy: isize) !void {
    ...
    comptime if (!meta.isNumeric(Al) or !meta.isNumeric(Be) or
        ...
        @compileError("zsl.linalg.blas.spmv: alpha and beta must be numerics, ap and x must be many-item pointers to numerics, and y must be a mutable many-item pointer to numerics, got \n\talpha: " ++ @typeName(Al) ++ "\n\tap: " ++ @typeName(Ap) ++ "\n\tx: " ++ @typeName(X) ++ "\n\tbeta: " ++ @typeName(Be) ++ "\n\ty: " ++ @typeName(Y) ++ "\n");
    ...
    if (comptime options.link_cblas != null and Al == Ap and Al == X and Al == Y and Al == Be) {
        switch (comptime meta.numericType(Al)) {
            .float => {
                if (comptime Al == f32)
                    return linalg.cblas.sspmv(layout.toInt(c_int), uplo.toInt(c_int), numeric.cast(isize, n), &alpha, ap, x, incx, &beta, y, incy)
                else if (comptime Al == f64)
                    return linalg.cblas.dspmv(layout.toInt(c_int), uplo.toInt(c_int), numeric.cast(isize, n), &alpha, ap, x, incx, &beta, y, incy);
            },
            else => {},
        }
    }
    ...
    return if (layout == .col_major)
        k_spmv(uplo, n, alpha, ap, x, incx, beta, y, incy)
    else
    ...
    if (uplo == .upper) {
        ...
        if (incx == 1 and incy == 1) {
            ...
            while (j < n) : (j += 1) {
                ...
                var temp2 = numeric.zero(meta.Accumulator(numeric.Mul(Ap, X)));

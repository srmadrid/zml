        ...
        fn execute(
            ...
            while (true) {
                ...
                if (tile_i == tile_j) {
                    ...
                    // Fast path: avoid copying if incx == 1
                    const px = if (worker_incx == 1) worker_x + r_start else blk: {
                    ...
                    const px_r = if (worker_incx == 1) worker_x + r_start else blk: {
                        @import("copy.zig").k_copy(
                            r_len,
                            worker_x + numeric.cast(usize, if (worker_incx > 0)
                                numeric.cast(isize, r_start) * worker_incx
                    ...
                    const px_c = if (worker_incx == 1) worker_x + c_start else blk: {
                        @import("copy.zig").k_copy(
                            c_len,
                            worker_x + numeric.cast(usize, if (worker_incx > 0)
                        ...
                        while (i < (r_len / unroll) * unroll) : (i += unroll) {
                            inline for (0..unroll) |u| {
                                numeric.fma_(
                                    ...
                                    local_y_r[i + u], // Bug fix applied here
                                );
...
pub fn k_hemv(uplo: Uplo, n: usize, alpha: anytype, a: anytype, lda: usize, x: anytype, incx: isize, beta: anytype, y: anytype, incy: isize, comptime noconj: bool) void {
    const Al: type = @TypeOf(alpha);
    ...
    if (uplo == .upper) {
        ...
        if (incx == 1 and incy == 1) {
            var j: usize = 0;
            while (j < n) : (j += 1) {
                const temp1 = numeric.mul(alpha, x[j]);
                ...
                while (i < (j / unroll) * unroll) : (i += unroll) {
                    inline for (0..unroll) |u| {
                        numeric.fma_(&y[i + u], temp1, if (comptime noconj) a[i + u + j * lda] else numeric.conj(a[i + u + j * lda]), y[i + u]);
                        numeric.fma_(&temp2, if (comptime !noconj) a[i + u + j * lda] else numeric.conj(a[i + u + j * lda]), x[i + u], temp2);
                    }
                }

                while (i < j) : (i += 1) {
                    numeric.fma_(&y[i], temp1, if (comptime noconj) a[i + j * lda] else numeric.conj(a[i + j * lda]), y[i]);
                    numeric.fma_(&temp2, if (comptime !noconj) a[i + j * lda] else numeric.conj(a[i + j * lda]), x[i], temp2);
                }

                numeric.fma_(&y[j], temp1, numeric.re(a[j + j * lda]), numeric.fma(alpha, temp2, y[j]));
            }
        } else {
            ...
            while (tile_i < n) : (tile_i += tile_size) {
                ...
                while (j < n) : (j += 1) {
                    if (j < tile_i + b_len) {
                        const temp1 = numeric.mul(alpha, local_x[j - tile_i]);
                        var temp2 = numeric.zero(meta.Accumulator(numeric.Mul(if (comptime noconj) A else numeric.Conj(A), X)));
                        var sums: [unroll]meta.Accumulator(numeric.Mul(if (comptime noconj) A else numeric.Conj(A), X)) = .{numeric.zero(meta.Accumulator(numeric.Mul(if (comptime noconj) A else numeric.Conj(A), X)))} ** unroll;

                        const end_i = j - tile_i;
                        var i: usize = 0;
                        while (i + unroll <= end_i) : (i += unroll) {
                            inline for (0..unroll) |u| {
                                numeric.fma_(&local_y[i + u], temp1, if (comptime noconj) a[tile_i + i + u + j * lda] else numeric.conj(a[tile_i + i + u + j * lda]), local_y[i + u]);
                                numeric.fma_(&sums[u], if (comptime !noconj) a[tile_i + i + u + j * lda] else numeric.conj(a[tile_i + i + u + j * lda]), local_x[i + u], sums[u]);
                            }
                        }
                        ...
                        while (i < end_i) : (i += 1) {
                            numeric.fma_(&local_y[i], temp1, if (comptime noconj) a[tile_i + i + j * lda] else numeric.conj(a[tile_i + i + j * lda]), local_y[i]);
                            numeric.fma_(&temp2, if (comptime !noconj) a[tile_i + i + j * lda] else numeric.conj(a[tile_i + i + j * lda]), local_x[i], temp2);
                        }

                        numeric.fma_(&local_y[end_i], temp1, numeric.re(a[j + j * lda]), numeric.fma(alpha, temp2, local_y[end_i]));
                    } else {
                        ...
                        numeric.fma_(&y[numeric.cast(usize, jy)], alpha, temp2, y[numeric.cast(usize, jy)]);
                    }
        ...
        if (incx == 1 and incy == 1) {
            ...
            while (j < n) : (j += 1) {
                ...
                numeric.fma_(&y[j], temp1, numeric.re(a[j + j * lda]), y[j]);
                ...
                while (i < (n / unroll) * unroll) : (i += unroll) {
                    inline for (0..unroll) |u| {
                        numeric.fma_(&y[i + u], temp1, if (comptime noconj) a[i + u + j * lda] else numeric.conj(a[i + u + j * lda]), y[i + u]);
                        numeric.fma_(&sums[u], if (comptime !noconj) a[i + u + j * lda] else numeric.conj(a[i + u + j * lda]), x[i + u], sums[u]);
                    }
                }
                ...
                numeric.fma_(&y[j], alpha, temp2, y[j]);
            }
        } else {
            const tile_size = tileSize(X);
            var tile_i: usize = 0;
            while (tile_i < n) : (tile_i += tile_size) {
                ...
                while (j < tile_i + b_len) : (j += 1) {
                    if (j >= tile_i) {
                        ...
                        numeric.fma_(&local_y[j - tile_i], temp1, numeric.re(a[j + j * lda]), local_y[j - tile_i]);
                        ...
                        numeric.fma_(&local_y[j - tile_i], alpha, temp2, local_y[j - tile_i]);
                    } else {
                        const temp1 = numeric.mul(alpha, x[numeric.cast(usize, jx)]);
                        var temp2 = numeric.zero(meta.Accumulator(numeric.Mul(if (comptime noconj) A else numeric.Conj(A), X)));
                        var sums: [unroll]meta.Accumulator(numeric.Mul(if (comptime noconj) A else numeric.Conj(A), X)) = .{numeric.zero(meta.Accumulator(numeric.Mul(if (comptime noconj) A else numeric.Conj(A), X)))} ** unroll;

...
pub fn hemv(
    ...
    const tile_size = int.min(tileSize(X), tileSize(Y)) / 2;
...
fn k_hemv(uplo: Uplo, n: usize, alpha: anytype, a: anytype, lda: usize, x: anytype, incx: isize, beta: anytype, y: anytype, incy: isize, comptime noconj: bool) void {
    ...
    if (uplo == .upper) {
        ...
        if (incx == 1 and incy == 1) {
            ...
            const tile_size = int.min(tileSize(X), tileSize(Y));
                ...
                while (j < n) : (j += 1) {
                    if (j < tile_i + b_len) {
                        ...
                        while (i < ((j - tile_i) / unroll) * unroll) : (i += unroll) {
                            inline for (0..unroll) |u| {
                                // local_y[i + u] += temp1 * a[tile_i + i + u + j * lda]
                                numeric.fma_(
                                    &local_y[i + u],
                                    temp1,
                                    if (comptime noconj)
                                ...
                                // sums[u] += conj(a[tile_i + i + u + j * lda]) * local_x[i + u]
                                numeric.fma_(
                                    &sums[u],
                                    if (comptime !noconj)
                                        a[tile_i + i + u + j * lda]
                        ...
                        while (i < j - tile_i) : (i += 1) {
                            // local_y[i] += temp1 * a[tile_i + i + j * lda]
                            numeric.fma_(
                                &local_y[i],
                                temp1,
                                if (comptime noconj)
                            ...
                            // temp2 += conj(a[tile_i + i + j * lda]) * local_x[i]
                            numeric.fma_(
                                &temp2,
                                if (comptime !noconj)
                                    a[tile_i + i + j * lda]
                        ...
                        // local_y[j - tile_i] += temp1 * re(a[j + j * lda]) + alpha * temp2
                        numeric.fma_(
                            &local_y[j - tile_i],
                            temp1,
                            numeric.re(a[j + j * lda]),
                        ...
                        while (i + unroll <= b_len) : (i += unroll) {
                            inline for (0..unroll) |u| {
                                ...
                                // sums[u] += conj(a[tile_i + i + u + j * lda])
                                numeric.fma_(
                                    &sums[u],
                                    if (comptime !noconj)
                                        a[tile_i + i + u + j * lda]
            ...
            while (tile_i < n) : (tile_i += tile_size) {
                ...
                while (j < tile_i + b_len) : (j += 1) {
                    if (j >= tile_i) {
                        ...
                        // local_y[j - tile_i] += temp1 * re(a[j + j * lda])
                        numeric.fma_(
                            &local_y[j - tile_i],
                            temp1,
                            numeric.re(a[j + j * lda]),

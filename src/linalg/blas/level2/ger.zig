...
/// ## Signature
/// ```zig
/// linalg.blas.gerParallel(layout: matrix.Layout, m: usize, n: usize, alpha: Al, x: [*]const X, incx: isize, y: [*]const Y, incy: isize, a: [*]A, lda: usize, pool: *thread.Pool) !void
/// ```
///
/// ## Arguments
/// * `layout` (`matrix.Layout`): Specifies whether two-dimensional array
...
pub fn gerParallel(
    ...
    comptime if (!meta.isNumeric(Al) or
        ...
        !meta.isManyItemPointer(Y) or !meta.isNumeric(meta.Child(Y)) or
        !meta.isManyItemPointer(A) or meta.isConstPointer(A) or !meta.isNumeric(meta.Child(A)))
        @compileError("zsl.linalg.blas.gerParallel: alpha must be a numeric, x and y must be many-item pointers to numerics, and a must be a mutable many-item pointer to numerics, got \n\talpha: " ++ @typeName(Al) ++ "\n\tx: " ++ @typeName(X) ++ "\n\ty: " ++ @typeName(Y) ++ "\n\ta: " ++ @typeName(A) ++ "\n");
    ...
    const Ctx = struct {
        ...
        fn kernel(ctx: @This(), start: usize, end: usize, worker_id: usize) void {
            ...
            if (ctx.iscolm)
                k_ger(worker_m, worker_n, ctx.alpha, worker_x, ctx.incx, worker_y, ctx.incy, worker_a, ctx.lda)
            else
                k_ger(worker_m, worker_n, ctx.alpha, worker_y, ctx.incy, worker_x, ctx.incx, worker_a, ctx.lda);
        }
    };

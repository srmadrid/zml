...
/// Solves a triangular matrix equation defined as;
///
/// ```zig
///     op(A) * X = alpha * B,
/// ```
...
/// is an `m`-by-`m` or `n`-by-`n` matrix, and `B` and `X` are `m`-by-`n`
/// matrices.
///
/// ## Signature
/// ```zig
/// linalg.blas.trsm(layout: matrix.Layout, side: linalg.blas.Side, uplo: matrix.Uplo, transa: linalg.blas.Transpose, diag: matrix.Diag, m: usize, n: usize, alpha: Al, a: [*]const A, lda: usize, b: [*]B, ldb: usize) !void
/// ```
///
/// ## Parameters
...
///   performed on `A`:
///   * `no_transpose`: `A * X  = alpha * B` or `X * A = alpha * B`
///   * `transpose`: `Aᵀ * X = alpha * B` or `X * Aᵀ = alpha * B`
///   * `conj_no_transpose`: `conj(A) * X = alpha * B` or `B * conj(A) = alpha * B`
///   * `conj_transpose`: `Aᴴ * X = alpha * B` or `B * Aᴴ = alpha * B`
...
///   On entry, must contain the right hand side matrix `B`, on return, contains
...
pub fn trsm(
    ...
    comptime if (!meta.isNumeric(Al) or
        ...
        @compileError("zsl.linalg.blas.trsm: alpha must be a numeric, a must be a many-item pointer to numerics, and b must be a mutable many-item pointer to numerics, got \n\talpha: " ++ @typeName(Al) ++ "\n\ta: " ++ @typeName(A) ++ "\n\tb: " ++ @typeName(B) ++ "\n");
    ...
    if (layout == .col_major) {
        ...
        return if (noconja)
            k_trsm(side, uplo, transa, diag, m, n, alpha, a, lda, b, ldb, true)
        else
            k_trsm(side, uplo, transa, diag, m, n, alpha, a, lda, b, ldb, false);
    } else {
        ...
        return if (noconja)
            k_trsm(side.invert(), uplo.invert(), transa, diag, n, m, alpha, a, lda, b, ldb, true)
        else
            k_trsm(side.invert(), uplo.invert(), transa, diag, n, m, alpha, a, lda, b, ldb, false);
    }
}
                ...
                for (0..n) |j| {
                    ...
                    while (k > 0) {
                        ...
                        if (numeric.ne(b[k + j * ldb], 0)) {
                            if (diag == .non_unit) {
                                numeric.divInto( // b[k + j * ldb] /= a[k + k * lda]
                                    &b[k + j * ldb],
                                    b[k + j * ldb],
                                    if (comptime noconja)
                                        a[k + k * lda]
            ...
            if (uplo == .upper) {
                for (0..n) |j| {
                    for (0..m) |i| {
                        ...
                        var temp = numeric.cast(
                            meta.Accumulator(numeric.Mul(A, B)),
                            numeric.mul(
                                b[i + j * ldb],
                                alpha,
                            ),
                ...
                while (k > 0) {
                    ...
                    for (0..k) |j| {
                        if (numeric.ne(a[j + k * lda], 0)) {
                            const temp = if (comptime noconja)
                                a[j + k * lda]
                            else

...
/// Performs in-place computation of the log-gamma function at `x`,
/// `log(∫₀^∞ tˣ⁻¹ e⁻ᵗ dt)`.
///
/// The `lgamma_` routine computes the log-gamma function at its input `x`,
/// `log(∫₀^∞ tˣ⁻¹ e⁻ᵗ dt)` and stores the result directly into `o`,
...
/// - **Numeric = lgamma(Numeric)**: scalar log-gamma function.
/// - **Matrix = lgamma(Matrix)**: matrix log-gamma function (not implemented yet).
/// - **Array = lgamma(Array)**: element-wise log-gamma function.
/// - **Expression = lgamma(Expression)**: symbolic log-gamma function.
///
/// Signature
/// ---------
/// ```zig
/// fn lgamma_(o: *O, x: X, ctx: anytype) !void
/// ```
///
/// Parameters
/// ----------
...
/// avoiding repeated allocations in scenarios where `lgamma_` is called multiple
...
pub inline fn lgamma_(
    ...
    comptime if (!types.isArray(O) and !types.isArray(X) and
        ...
        @compileError("zml.lgamma_ not defined for " ++ @typeName(O) ++ " and " ++ @typeName(X));

    switch (comptime types.domainType(O)) {
        .array => switch (comptime types.domainType(X)) {
            .array => { // array = lgamma(array)
                comptime switch (types.numericType(types.Numeric(O))) {
                    .bool, .int, .float, .cfloat => switch (types.numericType(types.Numeric(X))) {
                        ...
                        else => @compileError("zml.lgamma_ between " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " not implemented yet"),
                    },
                    else => @compileError("zml.lgamma_ between " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " not implemented yet"),
                };

                return array.lgamma_(
                    o,
                    x,
                    ctx,
                );
            },
            else => @compileError("zml.lgamma_ not defined for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
        },
        .numeric => switch (comptime types.domainType(X)) {
            .numeric => { // numeric = lgamma(numeric)
                switch (comptime types.numericType(O)) {
                    .bool, .int, .float, .cfloat => switch (comptime types.numericType(X)) {
                        ...
                        .integer => @compileError("zml.lgamma_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                        .rational => @compileError("zml.lgamma_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                        .real => @compileError("zml.lgamma_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                        .complex => @compileError("zml.lgamma_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                    },
                    else => @compileError("zml.lgamma_ between " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " not implemented yet"),
                }
            },

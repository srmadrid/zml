...
/// Performs in-place computation of the gamma function at `x`,
/// `∫₀^∞ tˣ⁻¹ e⁻ᵗ dt`.
///
/// The `gamma_` routine computes the gamma function at its input `x`,
/// `∫₀^∞ tˣ⁻¹ e⁻ᵗ dt` and stores the result directly into `o`, automatically
/// validating the provided context. The operation is performed in the input's
/// precision, and the result is then cast to the output type. It supports both
/// fixed-precision and arbitrary-precision arithmetic, as well as structured
/// data domains. The supported type combinations are:
/// - **Numeric = gamma(Numeric)**: scalar gamma function.
/// - **Matrix = gamma(Matrix)**: matrix gamma function (not implemented yet).
/// - **Array = gamma(Array)**: element-wise gamma function.
/// - **Expression = gamma(Expression)**: symbolic gamma function.
///
/// Signature
/// ---------
/// ```zig
/// fn gamma_(o: *O, x: X, ctx: anytype) !void
/// ```
///
/// Parameters
/// ----------
...
///
/// `x` (`anytype`):
/// The operand to compute the gamma function at.
///
/// `ctx` (`anytype`):
...
/// avoiding repeated allocations in scenarios where `gamma_` is called multiple
...
pub inline fn gamma_(
    ...
    comptime if (!types.isArray(O) and !types.isArray(X) and
        ...
        @compileError("zml.gamma_ not defined for " ++ @typeName(O) ++ " and " ++ @typeName(X));

    switch (comptime types.domain(O)) {
        .array => switch (comptime types.domain(X)) {
            .array => { // array = gamma(array)
                comptime switch (types.numericType(types.Numeric(O))) {
                    .bool, .int, .float, .cfloat => switch (types.numericType(types.Numeric(X))) {
                        ...
                        else => @compileError("zml.gamma_ between " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " not implemented yet"),
                    },
                    else => @compileError("zml.gamma_ between " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " not implemented yet"),
                };
            ...
            else => @compileError("zml.gamma_ not defined for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
        },
        .numeric => switch (comptime types.domain(X)) {
            .numeric => { // numeric = gamma(numeric)
                switch (comptime types.numericType(O)) {
                    .bool, .int, .float, .cfloat => switch (comptime types.numericType(X)) {
                        ...
                        .integer => @compileError("zml.gamma_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                        .rational => @compileError("zml.gamma_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                        .real => @compileError("zml.gamma_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                        .complex => @compileError("zml.gamma_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                    },
                    else => @compileError("zml.gamma_ between " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " not implemented yet"),
                }
            },

...
/// Performs in-place computation of the arccosine of `x`.
///
/// The `acos_` routine computes the arccosine of its input `x`, and stores the
...
/// combinations are:
/// - **Numeric = acos(Numeric)**: scalar arccosine.
/// - **Matrix = acos(Matrix)**: matrix arccosine (not implemented yet).
/// - **Array = acos(Array)**: element-wise arccosine.
/// - **Expression = acos(Expression)**: symbolic arccosine.
///
/// Signature
/// ---------
/// ```zig
/// fn acos_(o: *O, x: X, ctx: anytype) !void
/// ```
///
/// Parameters
/// ----------
...
///
/// `x` (`anytype`):
/// The operand to compute the arccosine of.
///
/// `ctx` (`anytype`):
...
/// avoiding repeated allocations in scenarios where `acos_` is called multiple
...
pub inline fn acos_(
    ...
    comptime if (!types.isArray(O) and !types.isArray(X) and
        ...
        @compileError("zml.acos_ not defined for " ++ @typeName(O) ++ " and " ++ @typeName(X));
    ...
    switch (comptime types.domainType(O)) {
        .array => switch (comptime types.domainType(X)) {
            .array => { // array = acos(array)
                comptime switch (types.numericType(types.Numeric(O))) {
                    .bool, .int, .float, .cfloat => switch (types.numericType(types.Numeric(X))) {
                        ...
                        else => @compileError("zml.acos_ between " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " not implemented yet"),
                    },
                    else => @compileError("zml.acos_ between " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " not implemented yet"),
                };
            ...
            else => @compileError("zml.acos_ not defined for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
        },
            ...
            .numeric => { // numeric = acos(numeric)
                switch (comptime types.numericType(O)) {
                    .bool, .int, .float, .cfloat => switch (comptime types.numericType(X)) {
                        ...
                        .integer => @compileError("zml.acos_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                        .rational => @compileError("zml.acos_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                        .real => @compileError("zml.acos_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                        .complex => @compileError("zml.acos_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                    },
                    else => @compileError("zml.acos_ between " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " not implemented yet"),
                }
            },

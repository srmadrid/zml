...
/// Performs in-place computation of the cosine of `x`.
///
/// The `cos_` routine computes the cosine of its input `x`, and stores the
...
/// combinations are:
/// - **Numeric = cos(Numeric)**: scalar cosine.
/// - **Matrix = cos(Matrix)**: matrix cosine (not implemented yet).
/// - **Array = cos(Array)**: element-wise cosine.
/// - **Expression = cos(Expression)**: symbolic cosine.
///
/// Signature
/// ---------
/// ```zig
/// fn cos_(o: *O, x: X, ctx: anytype) !void
/// ```
///
/// Parameters
/// ----------
...
/// avoiding repeated allocations in scenarios where `cos_` is called multiple
            ...
            .numeric => { // numeric = cos(numeric)
                switch (comptime types.numericType(O)) {
                    .bool, .int, .float, .cfloat => switch (comptime types.numericType(X)) {
                        ...
                        .integer => @compileError("zml.cos_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                        .rational => @compileError("zml.cos_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                        .real => @compileError("zml.cos_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                        .complex => @compileError("zml.cos_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                    },
                    else => @compileError("zml.cos_ between " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " not implemented yet"),
                }
            },

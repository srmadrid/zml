...
pub inline fn acosh_(
    ...
    switch (comptime types.domainType(O)) {
        .array => switch (comptime types.domainType(X)) {
            .array, .numeric => { // array = acosh(numeric), array = acosh(array)
                ...
                switch (comptime types.numericType(X)) {
                    ...
                    .int => {
                        comptime if (@typeInfo(X).int.signedness == .unsigned)
                            @compileError("zml.acosh_ not defined for unsigned integers, got " ++ @typeName(X));

...
/// Performs in-place computation of the hyperbolic sine of `x`.
///
/// The `sinh_` routine computes the hyperbolic sine of its input `x`, and
...
/// - **Numeric = sinh(Numeric)**: scalar hyperbolic sine.
/// - **Matrix = sinh(Matrix)**: matrix hyperbolic sine (not implemented yet).
/// - **Array = sinh(Array)**: element-wise hyperbolic sine.
/// - **Expression = sinh(Expression)**: symbolic hyperbolic sine.
///
/// Signature
/// ---------
/// ```zig
/// fn sinh_(o: *O, x: X, ctx: anytype) !void
/// ```
///
/// Parameters
/// ----------
...
/// avoiding repeated allocations in scenarios where `sinh_` is called multiple
            ...
            .numeric => { // numeric = sinh(numeric)
                switch (comptime types.numericType(O)) {
                    .bool, .int, .float, .cfloat => switch (comptime types.numericType(X)) {
                        ...
                        .integer => @compileError("zml.sinh_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                        .rational => @compileError("zml.sinh_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                        .real => @compileError("zml.sinh_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                        .complex => @compileError("zml.sinh_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                    },
                    else => @compileError("zml.sinh_ between " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " not implemented yet"),
                }
            },

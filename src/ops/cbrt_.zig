...
/// Performs in-place computation of the cube root of `x`.
///
/// The `cbrt_` routine computes the cube root of its input `x`, and stores the
...
/// combinations are:
/// - **Numeric = cbrt(Numeric)**: scalar cube root.
/// - **Matrix = cbrt(Matrix)**: matrix cube root (not implemented yet).
/// - **Array = cbrt(Array)**: element-wise cube root.
/// - **Expression = cbrt(Expression)**: symbolic cube root.
///
/// Signature
/// ---------
/// ```zig
/// fn cbrt_(o: *O, x: X, ctx: anytype) !void
/// ```
///
/// Parameters
/// ----------
...
/// avoiding repeated allocations in scenarios where `cbrt_` is called multiple
...
pub inline fn cbrt_(
    ...
    comptime if (!types.isArray(O) and !types.isArray(X) and
        ...
        @compileError("zml.cbrt_ not defined for " ++ @typeName(O) ++ " and " ++ @typeName(X));
    ...
    switch (comptime types.domain(O)) {
        .array => switch (comptime types.domain(X)) {
            .array => { // array = acos(array)
                comptime switch (types.numericType(types.Numeric(O))) {
                    .bool, .int, .float, .cfloat => switch (types.numericType(types.Numeric(X))) {
                        ...
                        else => @compileError("zml.cbrt_ between " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " not implemented yet"),
                    },
                    else => @compileError("zml.cbrt_ between " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " not implemented yet"),
                };
            ...
            else => @compileError("zml.cbrt_ not defined for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
        },
            ...
            .numeric => { // numeric = acos(numeric)
                switch (comptime types.numericType(O)) {
                    .bool, .int, .float, .cfloat => switch (comptime types.numericType(X)) {
                        ...
                        .integer => @compileError("zml.cbrt_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                        .rational => @compileError("zml.cbrt_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                        .real => @compileError("zml.cbrt_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                        .complex => @compileError("zml.cbrt_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                    },
                    else => @compileError("zml.cbrt_ between " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " not implemented yet"),
                }
            },

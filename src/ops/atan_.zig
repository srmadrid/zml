...
/// Performs in-place computation of the arctangent of `x`.
///
/// The `atan_` routine computes the arctangent of its input `x`, and stores the
...
/// combinations are:
/// - **Numeric = atan(Numeric)**: scalar arctangent.
/// - **Matrix = atan(Matrix)**: matrix arctangent (not implemented yet).
/// - **Array = atan(Array)**: element-wise arctangent.
/// - **Expression = atan(Expression)**: symbolic arctangent.
///
/// Signature
/// ---------
/// ```zig
/// fn atan_(o: *O, x: X, ctx: anytype) !void
/// ```
///
/// Parameters
/// ----------
...
/// avoiding repeated allocations in scenarios where `atan_` is called multiple
            ...
            .numeric => { // numeric = acos(numeric)
                switch (comptime types.numericType(O)) {
                    .bool, .int, .float, .cfloat => switch (comptime types.numericType(X)) {
                        ...
                        .integer => @compileError("zml.atan_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                        .rational => @compileError("zml.atan_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                        .real => @compileError("zml.atan_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                        .complex => @compileError("zml.atan_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                    },
                    else => @compileError("zml.atan_ between " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " not implemented yet"),
                }
            },

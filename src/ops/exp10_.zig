...
/// Performs in-place computation of the base-10 exponential of `x`, `10ˣ`.
///
/// The `exp10_` routine computes the base-10 exponential of its input `x`, `10ˣ`,
...
/// - **Numeric = exp10(Numeric)**: scalar base-10 exponential.
/// - **Matrix = exp10(Matrix)**: matrix base-10 exponential (not implemented yet).
/// - **Array = exp10(Array)**: element-wise base-10 exponential.
/// - **Expression = exp10(Expression)**: symbolic base-10 exponential.
///
/// Signature
/// ---------
/// ```zig
/// fn exp10_(o: *O, x: X, ctx: anytype) !void
/// ```
///
/// Parameters
/// ----------
...
/// avoiding repeated allocations in scenarios where `exp10_` is called multiple
            ...
            .numeric => { // numeric = exp10(numeric)
                switch (comptime types.numericType(O)) {
                    .bool, .int, .float, .cfloat => switch (comptime types.numericType(X)) {
                        ...
                        .integer => @compileError("zml.exp10_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                        .rational => @compileError("zml.exp10_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                        .real => @compileError("zml.exp10_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                        .complex => @compileError("zml.exp10_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                    },
                    else => @compileError("zml.exp10_ between " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " not implemented yet"),
                }
            },

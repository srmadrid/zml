...
/// Performs in-place computation of the error function at `x`,
/// `²/π ∫₀ˣ e^(−t²)dt`.
///
/// The `erf_` routine computes the error function at its input `x`,
/// `²/π ∫₀ˣ e^(−t²)dt` and stores the result directly into `o`, automatically
...
/// - **Numeric = erf(Numeric)**: scalar error function.
/// - **Matrix = erf(Matrix)**: matrix error function (not implemented yet).
/// - **Array = erf(Array)**: element-wise error function.
/// - **Expression = erf(Expression)**: symbolic error function.
///
/// Signature
/// ---------
/// ```zig
/// fn erf_(o: *O, x: X, ctx: anytype) !void
/// ```
///
/// Parameters
/// ----------
...
/// avoiding repeated allocations in scenarios where `erf_` is called multiple
            ...
            .array => { // array = erf(array)
                comptime switch (types.numericType(types.Numeric(O))) {
                    .bool, .int, .float, .cfloat => switch (types.numericType(types.Numeric(X))) {
                        ...
                        else => @compileError("zml.erf_ between " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " not implemented yet"),
                    },
                    else => @compileError("zml.erf_ between " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " not implemented yet"),
                };
            ...
            .numeric => { // numeric = erf(numeric)
                switch (comptime types.numericType(O)) {
                    .bool, .int, .float, .cfloat => switch (comptime types.numericType(X)) {
                        ...
                        .integer => @compileError("zml.erf_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                        .rational => @compileError("zml.erf_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                        .real => @compileError("zml.erf_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                        .complex => @compileError("zml.erf_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                    },
                    else => @compileError("zml.erf_ between " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " not implemented yet"),
                }
            },

...
/// The return type of the `ge` routine for inputs of types `X` and `Y`.
pub fn Ge(X: type, Y: type) type {
...
pub inline fn ge(
    ...
    switch (comptime types.domain(X)) {
        .expression => @compileError("zml.ge not implemented for expressions yet"),

...
pub inline fn re_(
    ...
    comptime if (!types.isArray(O) and !types.isArray(X) and
        ...
        @compileError("zml.re_ not defined for " ++ @typeName(O) ++ " and " ++ @typeName(X));
        ...
        .numeric => switch (comptime types.domainType(X)) {
            .numeric => { // numeric = re(numeric)
                switch (comptime types.numericType(X)) {
                    .bool => @compileError("zml.re_ not defined for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                    .int => {
                    ...
                    else => @compileError("zml.re_ between " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " not implemented yet"),
                }
            },
            else => @compileError("zml.re_ not defined for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
        },
        else => @compileError("zml.re_ not defined for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
    }
}

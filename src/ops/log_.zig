...
/// Performs in-place computation of the natural logarithm of `x`.
///
/// The `log_` routine computes the natural logarithm of its input `x`, and
...
/// - **Numeric = log(Numeric)**: scalar natural logarithm.
/// - **Matrix = log(Matrix)**: matrix natural logarithm (not implemented yet).
/// - **Array = log(Array)**: element-wise natural logarithm.
/// - **Expression = log(Expression)**: symbolic natural logarithm.
///
/// Signature
/// ---------
/// ```zig
/// fn log_(o: *O, x: X, ctx: anytype) !void
/// ```
///
/// Parameters
/// ----------
...
/// avoiding repeated allocations in scenarios where `log_` is called multiple
...
pub inline fn log_(
    ...
    comptime if (!types.isArray(O) and !types.isArray(X) and
        ...
        @compileError("zml.log_ not defined for " ++ @typeName(O) ++ " and " ++ @typeName(X));
    ...
    switch (comptime types.domain(O)) {
        .array => switch (comptime types.domain(X)) {
            .array => { // array = log(array)
                comptime switch (types.numericType(types.Numeric(O))) {
                    .bool, .int, .float, .cfloat => switch (types.numericType(types.Numeric(X))) {
                        ...
                        else => @compileError("zml.log_ between " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " not implemented yet"),
                    },
                    else => @compileError("zml.log_ between " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " not implemented yet"),
                };
            ...
            else => @compileError("zml.log_ not defined for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
        },
        .numeric => switch (comptime types.domain(X)) {
            .numeric => { // numeric = log(numeric)
                switch (comptime types.numericType(O)) {
                    .bool, .int, .float, .cfloat => switch (comptime types.numericType(X)) {
                        ...
                        .integer => @compileError("zml.log_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                        .rational => @compileError("zml.log_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                        .real => @compileError("zml.log_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                        .complex => @compileError("zml.log_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                    },
                    else => @compileError("zml.log_ between " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " not implemented yet"),
                }
            },

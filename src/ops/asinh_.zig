...
/// Performs in-place computation of the hyperbolic arcsine of `x`.
///
/// The `asinh_` routine computes the hyperbolic arcsine of its input `x`, and
...
/// - **Numeric = asinh(Numeric)**: scalar hyperbolic arcsine.
/// - **Matrix = asinh(Matrix)**: matrix hyperbolic arcsine (not implemented
///   yet).
/// - **Array = asinh(Array)**: element-wise hyperbolic arcsine.
/// - **Expression = asinh(Expression)**: hyperbolic symbolic arcsine.
///
/// Signature
/// ---------
/// ```zig
/// fn asinh_(o: *O, x: X, ctx: anytype) !void
/// ```
///
/// Parameters
/// ----------
...
/// avoiding repeated allocations in scenarios where `asinh_` is called multiple
...
pub inline fn asinh_(
    ...
    comptime if (!types.isArray(O) and !types.isArray(X) and
        ...
        @compileError("zml.asinh_ not defined for " ++ @typeName(O) ++ " and " ++ @typeName(X));
    ...
    switch (comptime types.domainType(O)) {
        .array => switch (comptime types.domainType(X)) {
            .array => { // array = acos(array)
                comptime switch (types.numericType(types.Numeric(O))) {
                    .bool, .int, .float, .cfloat => switch (types.numericType(types.Numeric(X))) {
                        ...
                        else => @compileError("zml.asinh_ between " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " not implemented yet"),
                    },
                    else => @compileError("zml.asinh_ between " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " not implemented yet"),
                };
            ...
            else => @compileError("zml.asinh_ not defined for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
        },
            ...
            .numeric => { // numeric = acos(numeric)
                switch (comptime types.numericType(O)) {
                    .bool, .int, .float, .cfloat => switch (comptime types.numericType(X)) {
                        ...
                        .integer => @compileError("zml.asinh_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                        .rational => @compileError("zml.asinh_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                        .real => @compileError("zml.asinh_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                        .complex => @compileError("zml.asinh_ not implemented yet for " ++ @typeName(O) ++ " and " ++ @typeName(X)),
                    },
                    else => @compileError("zml.asinh_ between " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " not implemented yet"),
                }
            },

...
pub fn Static(shape_: []const usize, N: type, order: array.Order) type {
    if (!meta.isNumeric(N))
    ...
    return struct {
        ...
        pub const ndim = shape_.len;
        pub const shape = blk: {
            var shape_arr: [array.max_dimensions]usize = .{0} ** array.max_dimensions;
            for (0..ndim) |i| {
        ...
        pub const strides = blk: {
            var strides_arr: [array.max_dimensions]usize = .{0} ** array.max_dimensions;
            var size_: usize = 1;
            for (0..ndim) |i| {
                const idx: usize = if (order == .f) i else ndim - i - 1;
                if (shape_[idx] == 1)
                    strides_arr[idx] = 0
                else
        ...
        pub const empty: array.Static(shape_, N, order) = .{
            .data = undefined,
        };
        ...
        pub fn initValue(value: N) array.Static(shape_, N, order) {
        ...
        /// ## Returns
        /// `array.Static(shape, N, order)`: The newly initialized array.
        pub fn initArray(values: [size]N) array.Static(shape_, N, order) {
            return .{ .data = values };
        }
        ...
        pub fn initFn(comptime @"fn": anytype, args: anytype) !array.Static(shape_, N, order) {
            const Fn = @TypeOf(@"fn");
            ...
            var arr: array.Static(shape_, N, order) = .init;
        ...
        pub fn get(self: array.Static(shape_, N, order), index: []const usize) !N {
            try arrutils.checkIndex(shape[0..ndim], index);
        ...
        /// ## Returns
        /// `N`: The element at the specified index.
        pub fn getAssumeInBounds(self: array.Static(shape_, N, order), index: []const usize) N {
        ...
        /// ## Errors
        /// * `array.Error.PositionOutOfBounds`: If `index` is out of bounds.
        pub fn set(self: *array.Static(shape_, N, order), index: []const usize, value: N) !void {
        ...
        /// ## Returns
        /// `void`
        pub fn setAssumeInBounds(self: *array.Static(shape_, N, order), index: []const usize, value: N) void {
        ...
        pub fn denseView(self: *array.Static(shape_, N, order)) array.Dense(N) {
            var dense_strides: [array.max_dimensions]isize = .{0} ** array.max_dimensions;
            for (0..ndim) |i| {
                dense_strides[i] = numeric.cast(isize, strides[i]);
            }
        ...
        pub fn format(self: *const array.Static(shape_, N, order), writer: *std.Io.Writer) !void {
            try self.formatter("{e}").format(writer);
        }

        pub fn formatter(self: *const array.Static(shape_, N, order), comptime num_fmt: []const u8) Formatter(num_fmt) {
            return .{ .array = self };
        }

        pub fn Formatter(comptime num_fmt: []const u8) type {
            return struct {
                array: *const array.Static(shape_, N, order),

                pub fn format(self: @This(), writer: *std.Io.Writer) !void {
                    try writer.writeAll("zsl.array.Static(&.{");
                    for (0..ndim) |i| {
                        if (i > 0) try writer.writeAll(", ");
                        try writer.print("{d}", .{shape[i]});
                    }
                    try writer.print("}}, {s}, .{s}) (", .{ @typeName(N), @tagName(order) });
                    for (0..ndim) |i| {
                        if (i > 0) try writer.writeAll(" × ");
                        try writer.print("{d}", .{shape[i]});
                    }
                    ...
                    return arrutils.format(self, num_fmt, shape[0..ndim], writer);
                }
            };
        }
    };

    ...
    if (depth == 0) {
        ...
        while (j < x.shape[idx]) : (j += 1) {
            if (comptime opinfo.@"fn".params.len == 3) {
                op_(&result.data[types.scast(u32, jr)], x.data[jx], y.data[jy]);
            } else if (comptime opinfo.@"fn".params.len == 4) {
                try op_(&result.data[types.scast(u32, jr)], x.data[jx], y.data[jy], ctx);
            }
...
pub fn apply2_(
    ...
    try loop2_(
        ...
        op_,
        o.ndim - 1,
        xx.flags.order.resolve2(yy.flags.order).toIterationOrder(),

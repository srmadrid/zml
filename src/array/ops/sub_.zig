...
pub inline fn sub_(
    ...
    comptime if (!types.isPointer(O) or types.isConstPointer(O))
        @compileError("zml.array.sub_ requires the output to be a mutable pointer, got " ++ @typeName(O));

    O = types.Child(O);

    if (comptime !types.isArray(O))
        @compileError("zml.array.sub_ requires the output to be an array, got " ++ @typeName(O));

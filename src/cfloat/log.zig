    ...
    switch (types.numericType(@TypeOf(z))) {
        ...
        .cfloat => {
            ...
            if (rcls == classify.ZERO and icls == classify.ZERO) {
                ...
                return .{
                    .re = -1 / float.abs(z.re),
                    .im = float.copysign(@as(Scalar(@TypeOf(z)), if (std.math.signbit(z.re)) float.pi(Scalar(@TypeOf(z))) else 0), z.im),
                };
                ...
                if (absr == 1 and scale == 0) {
                    ...
                    result.re = float.log(d) - cast(Scalar(@TypeOf(z)), scale, .{}) * float.ln2(Scalar(@TypeOf(z)));
                }

                result.im = float.atan2(z.im, z.re);

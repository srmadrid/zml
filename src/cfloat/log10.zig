...
pub fn log10(z: anytype) Cfloat(EnsureFloat(Scalar(@TypeOf(z)))) {
    comptime if (!types.isFixedPrecision(@TypeOf(z)))
        @compileError("cfloat.log10: z must be a bool, int, float or cfloat, got " ++ @typeName(@TypeOf(z)));
    ...
    if (rcls == classify.ZERO and icls == classify.ZERO) {
        ...
        return .{
            ...
            .im = float.copysign(@as(Scalar(@TypeOf(zz)), if (std.math.signbit(zz.re)) float.log10e(Scalar(@TypeOf(zz))) else 0), zz.im),
        };
        ...
        var result: Cfloat(Scalar(@TypeOf(zz))) = undefined;
        if (absr == 1 and scale == 0) {
            result.re = (float.log1p(absi * absi) * (@as(Scalar(@TypeOf(zz)), float.log10e(Scalar(@TypeOf(zz)))) / 2));
            ...
            if (absi >= std.math.floatEps(Scalar(@TypeOf(zz))))
                d2m1 += absi * absi;
            result.re = float.log1p(d2m1) * (@as(Scalar(@TypeOf(zz)), float.log10e(Scalar(@TypeOf(zz)))) / 2);
            ...
            result.re = float.log10(d) - scast(Scalar(@TypeOf(zz)), scale) * log10_2;
        }

        result.im = float.log10e(Scalar(@TypeOf(zz))) * float.atan2(zz.im, zz.re);

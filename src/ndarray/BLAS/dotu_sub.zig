...
pub inline fn dotu_sub(comptime T: type, n: isize, x: [*]const T, incx: isize, y: [*]const T, incy: isize, ret: *T) void {
    ...
    switch (supported) {
        .BuiltinBool => @compileError("BLAS.dotu_sub does not support bool."),
        .BuiltinInt => @compileError("BLAS.dotu_sub does not support integers. Use BLAS.dot instead."),
        .BuiltinFloat => @compileError("BLAS.dotu_sub does not support floats. Use BLAS.dot instead."),
        .Complex => {
...
test "dotu_sub" {
    ...
    BLAS.dotu_sub(Complex(f64), n, x1.ptr, 1, x2.ptr, 1, &result1);
    ...
    BLAS.dotu_sub(Complex(f64), n, x1.ptr, 1, x3.ptr, -1, &result2);
    ...
    BLAS.dotu_sub(Complex(f64), n / 2, x1.ptr, 2, x4.ptr, 2, &result3);

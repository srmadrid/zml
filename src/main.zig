...
fn perfTesting(a: std.mem.Allocator) !void {
    var A: zml.matrix.Tridiagonal(f64) = try .init(a, 6);
    defer A.deinit(a);
    ...
    var B: zml.matrix.Tridiagonal(f64) = try .init(a, 6);
    defer B.deinit(a);
    ...
    var C: zml.matrix.Tridiagonal(f64) = try zml.matrix.apply2(a, A, B, zml.add, .{});

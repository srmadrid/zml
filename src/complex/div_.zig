...
/// ---------
/// ```zig
/// fn div_(allocator: std.mem.Allocator, o: *Complex, x: X, y: Y, ctx: anytype) !void
/// ```
///
/// Parameters
/// ----------
...
pub fn div_(allocator: std.mem.Allocator, o: anytype, x: anytype, y: anytype) !void {
    ...
    switch (comptime types.numericType(X)) {
        .expression => @compileError("complex.div_ not implemented for Expression yet"),
        .complex => switch (comptime types.numericType(Y)) {
            .expression => @compileError("complex.div_ not implemented for Complex + Expression yet"),
            .complex => {
                if (x.num.size == 0) {
            ...
            .real => @compileError("complex.div_ not implemented for Complex + Real yet"),
            ...
            .bool => return div_(allocator, o, x, types.cast(X, y, .{}) catch unreachable),
        },
        .real => @compileError("complex.div_ not implemented for Real yet"),
        .rational => switch (comptime types.numericType(Y)) {
            .expression => @compileError("complex.div_ not implemented for Rational + Expression yet"),
            .complex => return div_(allocator, o, x.asComplex(), y),
            .real => @compileError("complex.div_ not implemented for Rational + Real yet"),
            .rational => return div_(allocator, o, x.asComplex(), y.asComplex()),
            .integer => return div_(allocator, o, x.asComplex(), y.asComplex()),
            .cfloat => {
                var ty = try @import("../cfloat/asComplex.zig").asComplex(y);
                ty[0].re.num.limbs = &ty[1][0];
                ty[0].re.den.limbs = &ty[1][1];
                ty[0].im.num.limbs = &ty[1][2];
                ty[0].im.den.limbs = &ty[1][3];
                return div_(allocator, o, x.asComplex(), ty[0]);
            },
            .float => {
                var ty = try @import("../float/asComplex.zig").asComplex(y);
                ty[0].re.num.limbs = &ty[1][0];
            ...
            .bool => return div_(allocator, o, x.asComplex(), types.cast(complex.Complex(rational.Rational), y, .{}) catch unreachable),
        },
        .integer => switch (comptime types.numericType(Y)) {
            .expression => @compileError("complex.div_ not implemented for Integer + Expression yet"),
            .complex => return div_(allocator, o, x.asComplex(), y),
            .real => @compileError("complex.div_ not implemented for Integer + Real yet"),
            .rational => return div_(allocator, o, x.asComplex(), y.asComplex()),
            .integer => return div_(allocator, o, x.asComplex(), y.asComplex()),
            .cfloat => {
                var ty = try @import("../cfloat/asComplex.zig").asComplex(y);
            ...
            .bool => {
                return div_(allocator, o, x.asComplex(), types.cast(complex.Complex(rational.Rational), y, .{}) catch unreachable);
            },
        },
        .cfloat => switch (comptime types.numericType(Y)) {
            .expression => @compileError("complex.div_ not implemented for CFloat + Expression yet"),
            .complex => {
            ...
            .real => @compileError("complex.div_ not implemented for CFloat + Real yet"),
            .rational => {
                var tx = try @import("../cfloat/asComplex.zig").asComplex(x);
                tx[0].re.num.limbs = &tx[1][0];
                tx[0].re.den.limbs = &tx[1][1];
                tx[0].im.num.limbs = &tx[1][2];
                tx[0].im.den.limbs = &tx[1][3];
                return div_(allocator, o, tx[0], y.asComplex());
            },
            .integer => {
                var tx = try @import("../cfloat/asComplex.zig").asComplex(x);
                tx[0].re.num.limbs = &tx[1][0];
            ...
            .bool => {
                ...
                tx[0].im.num.limbs = &tx[1][2];
                tx[0].im.den.limbs = &tx[1][3];
                return div_(allocator, o, tx[0], types.cast(complex.Complex(rational.Rational), y, .{}) catch unreachable);
            },
        },
        .float => switch (comptime types.numericType(Y)) {
            .expression => @compileError("complex.div_ not implemented for Float + Expression yet"),
            .complex => {
            ...
            .real => @compileError("complex.div_ not implemented for Float + Real yet"),
            .rational => {
                var tx = try @import("../float/asComplex.zig").asComplex(x);
                tx[0].re.num.limbs = &tx[1][0];
                tx[0].re.den.limbs = &tx[1][1];
        ...
        .int => switch (comptime types.numericType(Y)) {
            .expression => @compileError("complex.div_ not implemented for Int + Expression yet"),
            .complex => {
                var tx = @import("../int/asComplex.zig").asComplex(x);
                tx[0].re.num.limbs = &tx[1];
                return div_(allocator, o, tx[0], y);
            },
            .real => @compileError("complex.div_ not implemented for Int + Real yet"),
            .rational => {
                var tx = @import("../int/asComplex.zig").asComplex(x);
                tx[0].re.num.limbs = &tx[1];
                return div_(allocator, o, tx[0], y.asComplex());
        ...
        .bool => switch (comptime types.numericType(Y)) {
            .expression => @compileError("complex.div_ not implemented for Bool + Expression yet"),
            .complex => return div_(allocator, o, types.cast(complex.Complex(rational.Rational), x, .{}) catch unreachable, y),
            .real => @compileError("complex.div_ not implemented for Bool + Real yet"),
            .rational => return div_(allocator, o, types.cast(complex.Complex(rational.Rational), x, .{}) catch unreachable, y.asComplex()),
            .integer => return div_(allocator, o, types.cast(complex.Complex(rational.Rational), x, .{}) catch unreachable, y.asComplex()),
            .cfloat => {
                var ty = try @import("../cfloat/asComplex.zig").asComplex(y);
                ty[0].re.num.limbs = &ty[1][0];
                ty[0].re.den.limbs = &ty[1][1];
                ty[0].im.num.limbs = &ty[1][2];
                ty[0].im.den.limbs = &ty[1][3];
                return div_(allocator, o, types.cast(complex.Complex(rational.Rational), x, .{}) catch unreachable, ty[0]);
            },
            .float => {
                var ty = try @import("../float/asComplex.zig").asComplex(y);
                ty[0].re.num.limbs = &ty[1][0];

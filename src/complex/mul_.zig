...
/// Performs in-place multiplication between two operands of complex, real,
...
/// both operands to complex, then multiplying them in-place.
///
...
/// ## Signature
/// ```zig
/// complex.mul_(allocator: std.mem.Allocator, o: *O, x: X, y: Y) !void
/// ```
///
/// ## Arguments
...
pub fn mul_(allocator: std.mem.Allocator, o: anytype, x: anytype, y: anytype) !void {
    ...
    comptime if (!types.isPointer(O) or types.isConstPointer(O) or
        ...
        @compileError("zml.complex.mul_: o must be a mutable pointer to a complex, and at least one of x or y must be a complex, the other must be a bool, an int, a float, a dyadic, a cfloat, an integer, a rational, a real or a complex, got\n\to: " ++
            @typeName(O) ++ "\n\tx: " ++ @typeName(X) ++ "\n\ty: " ++ @typeName(Y) ++ "\n");
    ...
    switch (comptime types.numericType(X)) {
        .complex => switch (comptime types.numericType(Y)) {
            ...
            .real => return mul_(allocator, o, x, y.asComplex()),
        ...
        .real => return mul_(allocator, o, x.asComplex(), y),
        .rational => return mul_(allocator, o, x.asComplex(), y),
        .integer => return mul_(allocator, o, x.asComplex(), y),
        .cfloat => {

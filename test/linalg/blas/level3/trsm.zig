...
const trsm = zsl.linalg.blas.trsm;
...
fn executeTrsmTest(
    ...
    const rel_tol = 1e-8 * zsl.numeric.cast(f64, zsl.int.max(1, zsl.int.max(m, n)));
    for (b_expected, b_actual, 0..) |expected, actual, i| {
        if (N == f64) {
            ...
            if (diff_re > mag_re * rel_tol or diff_im > mag_im * rel_tol) {
                std.debug.print("\n\tTRSM Test Failed\n", .{});

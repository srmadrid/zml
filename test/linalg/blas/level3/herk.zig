...
const herk = zsl.linalg.blas.herk;

const tzsl = @import("../../../zsl.zig");

const combinations = .{
    // Empty arrays (all zero)
    .{ .col_major, .upper, .no_trans, @as(usize, 0), @as(usize, 0), @as(usize, 1), @as(usize, 1) },
    .{ .row_major, .lower, .conj_trans, @as(usize, 0), @as(usize, 0), @as(usize, 1), @as(usize, 1) },

    // k = 0 (Testing diagonal/beta scaling fallback)
    .{ .col_major, .lower, .no_trans, @as(usize, 10), @as(usize, 0), @as(usize, 10), @as(usize, 10) },
    .{ .row_major, .upper, .conj_trans, @as(usize, 12), @as(usize, 0), @as(usize, 12), @as(usize, 12) },

    // n = 0
    .{ .col_major, .upper, .no_trans, @as(usize, 0), @as(usize, 5), @as(usize, 1), @as(usize, 1) },

    // Single element
    .{ .col_major, .upper, .conj_trans, @as(usize, 1), @as(usize, 1), @as(usize, 1), @as(usize, 1) },
    ...
    .{ .row_major, .upper, .conj_trans, @as(usize, 64), @as(usize, 64), @as(usize, 64), @as(usize, 64) },
    ...
    .{ .col_major, .lower, .conj_trans, @as(usize, 15), @as(usize, 25), @as(usize, 25), @as(usize, 15) },
    .{ .row_major, .upper, .conj_trans, @as(usize, 17), @as(usize, 13), @as(usize, 17), @as(usize, 17) },
    ...
    .{ .row_major, .lower, .conj_trans, @as(usize, 10), @as(usize, 12), @as(usize, 14), @as(usize, 15) },

    // Large arrays
    .{ .col_major, .upper, .no_trans, @as(usize, 100), @as(usize, 120), @as(usize, 100), @as(usize, 100) },
    .{ .row_major, .lower, .conj_trans, @as(usize, 100), @as(usize, 120), @as(usize, 120), @as(usize, 100) },
    .{ .row_major, .lower, .no_trans, @as(usize, 1000), @as(usize, 1200), @as(usize, 1200), @as(usize, 1000) },

    // Prime-like numbers
    .{ .col_major, .lower, .conj_trans, @as(usize, 67), @as(usize, 73), @as(usize, 87), @as(usize, 77) },
...
fn executeHerkTest(
    ...
    const notrans = transa == .no_trans or transa == .conj_no_trans;
    ...
    const beta: f64 = tzsl.randomNumber(f64, rand);

...
const combinations = .{
    // Empty arrays
    .{ .col_major, .upper, .no_trans, .non_unit, @as(usize, 0), @as(usize, 1), @as(isize, 1), null, null },
    .{ .row_major, .lower, .trans, .unit, @as(usize, 0), @as(usize, 1), @as(isize, -1), null, null },

    // Single element
    .{ .row_major, .upper, .conj_no_trans, .unit, @as(usize, 1), @as(usize, 1), @as(isize, 1), null, null },
    .{ .col_major, .lower, .no_trans, .non_unit, @as(usize, 1), @as(usize, 1), @as(isize, -1), null, null },

    // Square aligned
    .{ .col_major, .upper, .no_trans, .non_unit, @as(usize, 32), @as(usize, 32), @as(isize, 1), null, null },
    .{ .row_major, .lower, .trans, .unit, @as(usize, 64), @as(usize, 64), @as(isize, 1), null, null },

    // Unaligned sizes
    .{ .col_major, .lower, .conj_trans, .non_unit, @as(usize, 37), @as(usize, 37), @as(isize, 1), null, null },
    .{ .row_major, .upper, .no_trans, .unit, @as(usize, 69), @as(usize, 69), @as(isize, 1), null, null },

    // Leading dimension variations (lda > n)
    .{ .col_major, .upper, .no_trans, .non_unit, @as(usize, 32), @as(usize, 40), @as(isize, 1), null, null },
    .{ .row_major, .lower, .trans, .unit, @as(usize, 37), @as(usize, 45), @as(isize, 1), null, null },

    // Strided vectors
    .{ .col_major, .upper, .no_trans, .non_unit, @as(usize, 32), @as(usize, 32), @as(isize, 2), null, null },
    .{ .row_major, .lower, .trans, .unit, @as(usize, 32), @as(usize, 32), @as(isize, -3), null, null },
    .{ .col_major, .lower, .conj_trans, .non_unit, @as(usize, 37), @as(usize, 40), @as(isize, -1), null, null },
    .{ .row_major, .upper, .conj_no_trans, .unit, @as(usize, 37), @as(usize, 40), @as(isize, 2), null, null },

    // Lowering parallel_threshold to test threading logic on small data chunks
    .{ .col_major, .upper, .no_trans, .non_unit, @as(usize, 64), @as(usize, 64), @as(isize, 1), null, @as(usize, 10) },
    .{ .row_major, .lower, .trans, .unit, @as(usize, 65), @as(usize, 65), @as(isize, 1), null, @as(usize, 8) },

    // Thin/Large matrices for parallel chunking
    .{ .col_major, .upper, .no_trans, .non_unit, @as(usize, 2000), @as(usize, 2000), @as(isize, 1), null, null },
    .{ .row_major, .lower, .trans, .unit, @as(usize, 1500), @as(usize, 1500), @as(isize, 2), null, null },

    // Forced multithreading
    .{ .col_major, .lower, .conj_trans, .non_unit, @as(usize, 65), @as(usize, 65), @as(isize, -1), @as(usize, 2), null },
    .{ .row_major, .upper, .no_trans, .unit, @as(usize, 65), @as(usize, 70), @as(isize, 2), @as(usize, 4), null },

    // Forced single-threaded fallback
    .{ .row_major, .upper, .conj_no_trans, .non_unit, @as(usize, 1000), @as(usize, 1000), @as(isize, 2), @as(usize, 1), null },
    .{ .col_major, .lower, .trans, .unit, @as(usize, 1500), @as(usize, 1505), @as(isize, 1), @as(usize, 1), null },

    // Explicit high thread count
    .{ .col_major, .upper, .no_trans, .non_unit, @as(usize, 1000), @as(usize, 1000), @as(isize, 1), @as(usize, 8), null },

    // Prime-like numbers to ensure threads get unequal chunks
    .{ .col_major, .lower, .no_trans, .unit, @as(usize, 1003), @as(usize, 1510), @as(isize, 1), @as(usize, 4), null },
};

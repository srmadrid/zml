...
test dotc {
    ...
    inline for (combinations) |combo| {
        ...
        inline for (.{ f64, zsl.cf64 }) |T| {
            try executeDotcTest(T, allocator, rand, n, incx, incy, num_threads, parallel_threshold);
        }
    }
}
...
fn executeDotcTest(
    ...
    const expected = if (T == f64)
        ...
        zsl.linalg.cblas.zdotc_sub(zsl.numeric.cast(isize, n), @ptrCast(x.ptr), zsl.numeric.cast(isize, incx), @ptrCast(y.ptr), zsl.numeric.cast(isize, incy), &result);
        break :blk result;
    };

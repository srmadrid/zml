...
test nrm2 {
    ...
    inline for (combinations) |combo| {
        ...
        inline for (.{ f64, zsl.cf64 }) |T| {
            try executeNrm2Test(T, allocator, rand, n, incx, num_threads, parallel_threshold);
        }
    }
}
...
fn executeNrm2Test(
    ...
    const expected: f64 = if (T == f64)
        zsl.linalg.cblas.dnrm2(zsl.numeric.cast(isize, n), @ptrCast(x.ptr), zsl.numeric.cast(isize, abs_incx))
    else
        zsl.linalg.cblas.dznrm2(zsl.numeric.cast(isize, n), @ptrCast(x.ptr), zsl.numeric.cast(isize, abs_incx));
